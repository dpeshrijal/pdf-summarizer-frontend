  ...
  const downloadAsPDF = async (text: string, filename: string) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (!nameFound) {
        ...
        doc.text(line.toUpperCase(), pageWidth / 2, yPosition, { align: 'center' });
        yPosition += 10;
        nameFound = true;
        continue;
      }
      ...
      if (!contactFound && (line.includes('@') || line.toLowerCase().includes('email'))) {
        doc.setFontSize(9);
        ...
        const linkedinMatch = line.match(/linkedin\.com\/in\/[\w-]+/i) || line.match(/LinkedIn:\s*([\w\s]+)/i);
        ...
        if (linkedinMatch) contactParts.push(linkedinMatch[0].replace('LinkedIn:', '').trim());
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const commonSections = ['SUMMARY', 'SKILLS', 'WORK EXPERIENCE', 'EXPERIENCE', 'CERTIFICATIONS', 'CERTIFICATION', 'EDUCATION', 'PROJECTS'];
      const isCommonSection = commonSections.includes(trimmedLine.toUpperCase());

      const isAllCaps = (trimmedLine === trimmedLine.toUpperCase() &&
                        trimmedLine.length > 3 &&
                        !trimmedLine.includes('|') &&
                        !trimmedLine.includes('•') &&
                        !/\d{4}/.test(trimmedLine)) || isCommonSection; // Not a date
      ...
      if (isAllCaps) {
        ...
        doc.line(marginLeft, yPosition + 1, marginLeft + headerWidth, yPosition + 1);
      ...
      const isSubheader = /^[A-Z][a-zA-Z\s&]+:\s*.+/.test(trimmedLine) && trimmedLine.split(':')[0].length < 50;
      ...
      if (isSubheader) {
        ...
        // Normal content
        doc.setFont('helvetica', 'normal');
        const contentLines = doc.splitTextToSize(content, maxLineWidth - headerWidth - 2);
      ...
      const isJobTitle = /^[A-Z][a-zA-Z\s,]+.*\(.*(\d{4}|present)/i.test(trimmedLine);
      ...
      if (isJobTitle) {
        ...
        const datePattern = /\((\d{4}\s*[-–—]\s*(?:\d{4}|present|Present))\)\s*$/i;
        const dateMatch = trimmedLine.match(datePattern);

        if (dateMatch) {
          // Get everything before the date parentheses
          const dateStartIndex = trimmedLine.lastIndexOf('(', trimmedLine.length - 1);
          const titleCompanyLocation = trimmedLine.substring(0, dateStartIndex).trim();
          ...
          const titleLines = doc.splitTextToSize(titleCompanyLocation, maxTitleWidth);
                 ...
                 <div className="space-y-8">
                    <div>
                        <div className="flex justify-between items-center mb-2">
                            ...
                            <button
                                onClick={() => downloadAsPDF(generatedDocs.tailoredResume, 'tailored-resume.pdf')}
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-2">
                            ...
                            <button
                                onClick={() => downloadAsPDF(generatedDocs.coverLetter, 'cover-letter.pdf')}
                                className="px-4 py-2 bg-purple-600 hover:bg-purple-700 rounded-md font-semibold text-sm transition-colors"
                            >
                                Download PDF
                            </button>
